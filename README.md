...
成率定义： 
```
单位时间内被调服务成功率 =  （单位时间内被调用结果为ture的次数）/ 单位时间内总被调用次数  【结果换算成百分比，小数点后保留两位有数字,不足两位补0，其他位直接舍弃，不进位】
以上示例数据计算一分钟的成功率
 - （2020-06-01 09:42）userServie在被调成功率: 4/6= 66.66%
 - （2020-06-01 09:42）userServie在172.17.60.4机器上成率: 2 / 4 = 50.00%
...
输出：返回在这一分钟内主被调按ip聚合的成功率和P99(按成功率倒排)
...
- 成绩评测过程
  ...
  - 查询阶段（checkPair）：执行查询次数N2次，总耗时T2
  - 查询阶段（checkResponder）：执行查询次数N3次，总耗时T3
- 如果查询结果都正确，成绩为 N1/T1 + N2/T2 + N3/T3
